...
def outpaint(image_path: str, generator: Generate, mask_path: str, prompt: str, name: str = '', **kwargs) -> str:
    ...
    output = generator.img2img(prompt=prompt, init_img=image_path, init_mask=mask_path, **kwargs)
    if name:
        os.move(output[0][0], name)
        return name
    return output[0][0]
...
def create_pixel_edge_image(direction:str,original_above_or_below_img_path: str, original_to_left_or_to_right_img_path: str) -> Image:
    ...
    if direction not in ['above', 'below', 'left', 'right', 'above_to_left', 'above_to_right', 'below_to_left', 'below_to_right']:
        raise ValueError('direction must be one of the following: above, below, left, right, above_to_left, above_to_right, below_to_left, below_to_right')
    ...
    if direction == 'above' or direction == 'below':
        if original_above_or_below_img_path is None:
            raise ValueError('original_above_or_below_img_path must be provided because of specified direction')

    if direction == 'left' or direction == 'right':
        if original_to_left_or_to_right_img_path is None:
            raise ValueError('original_to_left_or_to_right_img_path must be provided because of specified direction')
    ...
    if direction == 'above_to_left' or direction == 'above_to_right' or direction == 'below_to_left' or direction == 'below_to_right':
        if original_above_or_below_img_path is None or original_to_left_or_to_right_img_path is None:
            raise ValueError('original_above_or_below_img_path and original_to_left_or_to_right_img_path must be provided because of specified direction')

    # open supplied images
    if original_above_or_below_img_path is not None:
        original_above_or_below_img = Image.open(original_above_or_below_img_path).convert('RGBA')
        original_above_or_below_img_matrix = np.array(original_above_or_below_img)

    if original_to_left_or_to_right_img_path is not None:
        original_to_left_or_to_right_img = Image.open(original_to_left_or_to_right_img_path).convert('RGBA')
        original_to_left_or_to_right_img_matrix = np.array(original_to_left_or_to_right_img)

    #create new image to be returned
    transparent_img = Image.new('RGBA', (512, 512), (0, 0, 0, 0)) # make sure to change alpha back to zero in prod
    ...
    if direction == 'above' or direction == 'above_to_left' or direction == 'above_to_right':
        ...
        transparent_img_matrix[:, 342:512] = original_to_left_or_to_right_img_matrix[:,0:170]
        ...
        transparent_img_matrix[:, 0:170] = original_to_left_or_to_right_img_matrix[:, 342:512]
...
def create_partial_image_from_edge_filled_img_path(edge_filled_img: Image, location: Location, prompt: str, steps: int, **kwargs) -> str:
    ...
    edge_filled_img.save(f'{location.x}x_{location.y}y_pixel_edge.png')
    edge_filled_image = edgify(edge_filled_img)
    edge_filled_image.save(f'{location.x}x_{location.y}y_pixel_edge_filled.png')
    created_image_path = outpaint(f'{location.x}x_{location.y}y_pixel_edge_filled.png',sd, f'{location.x}x_{location.y}y_pixel_edge.png', prompt, steps=steps, height=512, width=512, **kwargs)
...
def create_image_above(below_img_path: str, location: Location, prompt: str, **kwargs) -> str:
    pixel_edge_img = create_pixel_edge_image(direction='above', original_above_or_below_img_path=below_img_path, original_to_left_or_to_right_img_path=None)
...
def create_image_below(above_img_path: str, location: Location, prompt: str, **kwargs) -> str:
    pixel_edge_img = create_pixel_edge_image(direction='below', original_above_or_below_img_path=above_img_path, original_to_left_or_to_right_img_path=None)
    return create_partial_image_from_edge_filled_img_path(edge_filled_img=pixel_edge_img, location=location, prompt=prompt, **kwargs)

def create_image_to_left(right_img_path: str, location: Location, prompt: str, **kwargs) -> str:
    pixel_edge_img = create_pixel_edge_image(direction='left', original_above_or_below_img_path=None, original_to_left_or_to_right_img_path=right_img_path)
    return create_partial_image_from_edge_filled_img_path(edge_filled_img=pixel_edge_img, location=location, prompt=prompt, **kwargs)

def create_image_to_right(left_img_path: str, location: Location, prompt: str, **kwargs) -> str:
    pixel_edge_img = create_pixel_edge_image(direction='right', original_above_or_below_img_path=None, original_to_left_or_to_right_img_path=left_img_path)
    return create_partial_image_from_edge_filled_img_path(edge_filled_img=pixel_edge_img, location=location, prompt=prompt, **kwargs)

def create_image_above_to_left(below_img_path: str, right_img_path: str, location: Location, prompt: str, **kwargs) -> str:
    pixel_edge_img = create_pixel_edge_image(direction='above_to_left', original_above_or_below_img_path=below_img_path, original_to_left_or_to_right_img_path=right_img_path)
    return create_partial_image_from_edge_filled_img_path(edge_filled_img=pixel_edge_img, location=location, prompt=prompt, **kwargs)

def create_image_above_to_right(below_img_path: str, left_img_path: str, location: Location, prompt: str, **kwargs) -> str:
    pixel_edge_img = create_pixel_edge_image(direction='above_to_right', original_above_or_below_img_path=below_img_path, original_to_left_or_to_right_img_path=left_img_path)
    return create_partial_image_from_edge_filled_img_path(edge_filled_img=pixel_edge_img, location=location, prompt=prompt, **kwargs)

def create_image_below_to_left(above_img_path: str, right_img_path: str, location: Location, prompt: str, **kwargs) -> str:
    pixel_edge_img = create_pixel_edge_image(direction='below_to_left', original_above_or_below_img_path=above_img_path, original_to_left_or_to_right_img_path=right_img_path)
    return create_partial_image_from_edge_filled_img_path(edge_filled_img=pixel_edge_img, location=location, prompt=prompt, **kwargs)

def create_image_below_to_right(above_img_path: str, left_img_path: str, location: Location, prompt: str, **kwargs) -> str:
    pixel_edge_img = create_pixel_edge_image(direction='below_to_right', original_above_or_below_img_path=above_img_path, original_to_left_or_to_right_img_path=left_img_path)
    return create_partial_image_from_edge_filled_img_path(edge_filled_img=pixel_edge_img, location=location, prompt=prompt, **kwargs)
...
def aggregate_images(completed_partial_images: List[PartialImage], width, height, upscale_factor=1) -> str:
    ...
    for partial_image in completed_partial_images:
        ...
        print(f'placing image {partial_image.location} at location', (x_location, y_location))
...
if __name__ == '__main__':
    ...
    for vertical_partial_image in range(max_up_down+1):
        for horizontal_partial_image in range(max_left_right+1):
            ...
            if vertical_partial_image == 0:
                collected_partial_images.append(PartialImage(image, Location(horizontal_partial_image, vertical_partial_image)))
                collected_partial_images.append(PartialImage(image, Location(-1*horizontal_partial_image, vertical_partial_image)))
            elif horizontal_partial_image == 0:
                collected_partial_images.append(PartialImage(image, Location(horizontal_partial_image, vertical_partial_image)))
                collected_partial_images.append(PartialImage(image, Location(horizontal_partial_image, -1*vertical_partial_image)))            
            else:
                collected_partial_images.append(PartialImage(image, Location(horizontal_partial_image, vertical_partial_image)))
                collected_partial_images.append(PartialImage(image, Location(-1*horizontal_partial_image, -1*vertical_partial_image)))
                collected_partial_images.append(PartialImage(image, Location(horizontal_partial_image, -1*vertical_partial_image)))
                collected_partial_images.append(PartialImage(image, Location(-1*horizontal_partial_image, vertical_partial_image)))
    ...
    for partial_image in collected_partial_images:
        ...
        print(f'{len(completed_partial_images)} images gererated of {len(collected_partial_images)}')
        ...
        def get_completed_partial_image_path(collected_partial_images: List[PartialImage], location: Location) -> str:
            ...
            raise Exception(f'Could not find completed partial image for location {location}')

        if above and left:
            completed_below_image_path = get_completed_partial_image_path(completed_partial_images, Location(partial_image.location.x, partial_image.location.y-1))
            completed_right_image_path = get_completed_partial_image_path(completed_partial_images, Location(partial_image.location.x+1, partial_image.location.y))
            partial_image.image_path = create_image_above_to_left(below_img_path=completed_below_image_path, right_img_path=completed_right_image_path, location=partial_image.location, prompt=args.prompt, steps=args.steps)
        elif above and right:
            completed_below_image_path = get_completed_partial_image_path(completed_partial_images, Location(partial_image.location.x, partial_image.location.y-1))
            completed_left_image_path = get_completed_partial_image_path(completed_partial_images, Location(partial_image.location.x-1, partial_image.location.y))
            partial_image.image_path = create_image_above_to_right(below_img_path=completed_below_image_path, left_img_path=completed_left_image_path, location=partial_image.location, prompt=args.prompt, steps=args.steps)
        elif below and left:
            completed_above_image_path = get_completed_partial_image_path(completed_partial_images, Location(partial_image.location.x, partial_image.location.y+1))
            completed_right_image_path = get_completed_partial_image_path(completed_partial_images, Location(partial_image.location.x+1, partial_image.location.y))
            partial_image.image_path = create_image_below_to_left(above_img_path=completed_above_image_path, right_img_path=completed_right_image_path, location=partial_image.location, prompt=args.prompt, steps=args.steps)
        elif below and right:
            completed_above_image_path = get_completed_partial_image_path(completed_partial_images, Location(partial_image.location.x, partial_image.location.y+1))
            completed_left_image_path = get_completed_partial_image_path(completed_partial_images, Location(partial_image.location.x-1, partial_image.location.y))
            partial_image.image_path = create_image_below_to_right(above_img_path=completed_above_image_path, left_img_path=completed_left_image_path, location=partial_image.location, prompt=args.prompt, steps=args.steps)
        elif above:
            completed_below_image_path = get_completed_partial_image_path(completed_partial_images, Location(partial_image.location.x, partial_image.location.y-1))             
            partial_image.image_path = create_image_above(below_img_path=completed_below_image_path, location=partial_image.location, prompt=args.prompt, steps=args.steps)
        elif below:
            completed_above_image_path = get_completed_partial_image_path(completed_partial_images, Location(partial_image.location.x, partial_image.location.y+1))
            partial_image.image_path = create_image_below(above_img_path=completed_above_image_path, location=partial_image.location, prompt=args.prompt, steps=args.steps)
        elif left:
            completed_right_image_path = get_completed_partial_image_path(completed_partial_images, Location(partial_image.location.x+1, partial_image.location.y))
            partial_image.image_path = create_image_to_left(right_img_path=completed_right_image_path, location=partial_image.location, prompt=args.prompt, steps=args.steps)
        elif right:
            completed_left_image_path = get_completed_partial_image_path(completed_partial_images, Location(partial_image.location.x-1, partial_image.location.y))
            partial_image.image_path = create_image_to_right(left_img_path=completed_left_image_path, location=partial_image.location, prompt=args.prompt, steps=args.steps)
        completed_partial_images.append(PartialImage(partial_image.image_path, partial_image.location))

        print('completed partial images so far: ', len(completed_partial_images))
        for completed_partial_image in completed_partial_images:
            print(f'Completed partial image: {completed_partial_image}')
